...
class MarketScraperWorkflow(BaseMiniApp):
    ...
    def execute(self, job: Any, on_log: Callable[[str], None]) -> Dict[str, Any]:
        ...
        csv_generated = False
        if csv_path:
            # Note: JobRunner adds artifacts if we had a method for it on 'job' object passed here?
            # JobRunner's '_execute_job' doesn't expose 'add_artifact' to the executor easily unless 'job' is the Job object.
            # Yes, 'job' argument IS the Job object.
